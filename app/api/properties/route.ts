...
// helper: server-side image dimension check using sharp
...
// If you want a public GET that returns only published properties, use the public endpoint below.
// This route is for owner-scoped listing (user's own properties) and POST to create new property.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // sanitize amenities & facilities (reuse your sanitize functions)
    ...
    const prop = await prisma.property.create({
      data: {
        ...
        images: {
          create: (imgs || []).map((img: any, idx: number) => ({
            url: img.url,
            alt: img.alt ?? null,
    ...
    console.error(err);
    return NextResponse.json({ error: "ServerError", message: String(err) }, { status: 500 });
  }
}